...
def windows():
    return gardenWindows()

def gardenWindows():
    dest=x(width-2850-wt)
    return util.flatten(map(lambda x: window.Window(x).parts(),
    [
        util.square(x(2850),z(wallMinHeight)).transO(dest),
        util.faceFromVectors([O,x(2850),xz(2850,heightAt(wt)), z(heightAt(2850))])

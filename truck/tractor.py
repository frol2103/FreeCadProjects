...
class Tractor:
 	floor 	=  {'width':Truck.width, 'length':Truck.cabine['length'], 'thickness':Truck.thickness}
	side  	= {'height':Truck.cabine['height'], 'length':floor['length'], 'thickness':0}
	...
	front 	= {'height':side['height']-floor['thickness']-top['thickness'], 'width':floor['width'], 'thickness':floor['thickness']}
	front.update({'windshield-bigedge':front['width']*1.5/10,'windshield-smalledge':front['width']/10,'windshield-topedge':front['height']/7,'windshield-botedge':front['height']*4/10})
	plate 	= {'voffset':floor['thickness'] ,'height':2*floor['thickness'], 'width':floor['width'], 'thickness':floor['thickness']}
	axis 	= {'height':Truck.axis['height'], 'width':(floor['width'])/3, 'length':Truck.axis['length']}
	wheel	= Truck.wheel
	fwheel	= Truck.fwheel
	axle	= Truck.axle
	axle.update({'width':floor['width']-2*wheel['width']})
	steering= {'xoffset':floor['length']*2/3,'yoffset':floor['width']/2,'axlePenetration':axle['side']/2.0}
	steering.update(Truck.steering)
	...
	def makePlate(self,pnt=Base.Vector(-plate['thickness'],0,-plate['height']+plate['voffset']),dir=Base.Vector(0, 0, 1)):
 	...
 	def makeAxis(self,pnt=Base.Vector(0,(floor['width']-axis['width'])/2,-axis['height']),dir=Base.Vector(0, 0, 1)):
		...
		#hole for the pivot
		hp = Part.makeCylinder(self.fwheel['innerHoleRadius'],self.axis['height'],Base.Vector(self.axis['length']-self.fwheel['bxoffset'],self.floor['width']/2,pnt.z),dir)
	...
	def makeBackAxle(self,pnt=Base.Vector(axis['length']-axle['bxoffset']-0.5*axle['side'],wheel['width'],-axis['height']-axle['side']),dir=Base.Vector(0, 0, 1)):
		p = Part.makeBox(self.axle['side'],self.axle['width'],self.axle['side'],pnt,dir) 
		w1 = self.makeWheel(pnt + Base.Vector(self.axle['side']/2.0, -self.wheel['width'],self.axle['side']/2.0))
		w2 = self.makeWheel(pnt + Base.Vector(self.axle['side']/2.0, self.axle['width'],self.axle['side']/2.0))
		return [p, w1, w2]
	...
	def make5Wheel(self,pnt=Base.Vector(axis['length']-fwheel['bxoffset'],floor['width']/2,0),dir=Base.Vector(0, 0, 1)):
